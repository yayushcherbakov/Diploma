...
using AuditService.ApplicationServices.Extensions;
...
internal class IdentityService : IIdentityService
{
    ...
    (
        ...
        IReadOnlyAuditDbContext readOnlyAuditDbContext,
    ...
    public async Task<AuthResponse> Authenticate(AuthRequest request, CancellationToken cancellationToken)
    {
        ...
        var roleIds = await _readOnlyAuditDbContext.UserRoles
            .Where(r => r.UserId == managedUser.Id)
            .Select(x => x.RoleId)
            .ToListAsync(cancellationToken);

        var roles = _readOnlyAuditDbContext.Roles.Where(x => roleIds.Contains(x.Id)).ToList();
        ...
        (RegisterUserPayload userPayload, CancellationToken cancellationToken)
    {
        ...
        var findUser = await _readOnlyAuditDbContext.Users.FirstOrDefaultAsync(
    ...
    {
        var usersInformation = await _readOnlyAuditDbContext
            .Users
        ...
        var total = await _readOnlyAuditDbContext.Users.CountAsync(cancellationToken);
